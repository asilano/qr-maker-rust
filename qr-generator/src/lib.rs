...
use std::error::Error;
use encoding_modes::EncodingModes;
...
impl QRGenerator {
	pub fn make_qr_code(&self, data: String) -> Result<String, Box<dyn Error>> {
	    // let data_codewords: Vec<u8> = encode_data_into_byte_stream(data);
	...
	fn save_qr_image(&self, filepath: &String, loud_region: GrayImage) -> Result<(), Box<dyn Error>> {
	...
	fn add_finder_patterns(buffer: &mut GrayImage, locations: Vec<FinderLocations>) {
	    for location in locations {
	        match location {
	            ...
	            FinderLocations::BottomLeft => Self::add_finder_pattern(buffer, 0, buffer.height() as i64 - 7),
	            FinderLocations::TopRight => Self::add_finder_pattern(buffer, buffer.width() as i64 - 7, 0)
	        }
	    }
	}
