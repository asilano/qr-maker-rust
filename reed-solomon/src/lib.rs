...
+ Rem<Output = CoeffType>
{
    ...
    pub fn encode(&self, mut message: Vec<Polynomial<CoeffType>>, ec_count: usize) -> Vec<Polynomial<CoeffType>> {
        ...
        let message_poly = Polynomial::from(
            message.into_iter().rev()
            .map(|p| self.galois_field.make_polynomial(p))
            .collect::<Vec<PolyWithinGF<Polynomial<CoeffType>>>>());
        ...
        let one = self.galois_field.make_polynomial(Polynomial::<CoeffType>::one());
        let mut generator = Polynomial::from(vec![negative_prim_pow, one.clone()]);
        for _ in 1..ec_count {
            primitive_power = &primitive_power * &self.galois_field.gf_primitive();
